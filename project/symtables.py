...
def getsymtables(initialInstr, finalInstr, blockLength):
    ...
    for blockLine in range(initialInstr,finalInstr+1):
        if (cg.is_int(cg.Instr3AC[blockLine].input1) == False and cg.Instr3AC[blockLine].input1!= ""):
        ...
        if (cg.is_int(cg.Instr3AC[blockLine].input2) == False and cg.Instr3AC[blockLine].input2!= ""):
