...
def p_statement(p):
    ...
    if(isinstance(p[1],dict)):
        if("loop" in p[1].keys()):
            sys.exit("ERROR: break statement out of loop")
            TAC.error = True
...
def p_M_17(p):
    ...
    if(funcdict==False):
        sys.exit('ERROR: '+str(p[-4])+' not defined')
        TAC.error = True
    elif(funcdict["num"]!=p[-2]):
        sys.exit('ERROR'+' Number of arguments passed mismatch in '+str(p[-4]))
        TAC.error = True
    ...
    if (isinstance(p[1],dict)):
        ...
        if (strType(p[1])=='int'):
            ...
            if (vardict == False):
                vardict = globalST.varlookup(p[1])
                if (vardict == False):
                    sys.exit('ERROR: '+str(p[1])+' not declared')
                    TAC.error = True
...
def p_M_10(p):
    ...
    TAC.emit("ifgoto", ["bg", temp_name, p[-2][1], label_name2])
...
def p_expr(p):
    ...
    '''
    if (p[3]["type"] == "string"):
        sys.exit("TYPE ERROR: variable "+str(p[3])+' string cannot be equated to variable.')
        TAC.error = True
    if(p[1]["type"] == "int" and p[3]["type"] != "int"):
        print("TYPE ERROR: variable "+str(p[1])+' and '+str(p[3])+' not matching type.')
        TAC.error = True
...
def p_mrhs(p):
	...
	if (len(p)==2):
		if p[1] == 'gets':
		    temp_name = ST.newtemp({"type":"int"})
		    TAC.emit('scan', [temp_name])
		    p[0] = {"place": temp_name, "type": "int"}
		else:
		    p[0] = p[1]
	elif (len(p) == 3):
		temp_name = ST.newtemp({"type" : "array"}, "yes")
